...
$facturas = array_filter($facturas, function ($factura) {
    return isset($factura['origen']) && strtoupper($factura['origen']) === 'VENTA';
});
?>
...
<head>
    <meta charset="UTF-8">
    <title>Facturas de Ventas- <?= htmlspecialchars($empresa['razon_social']) ?> (<?= htmlspecialchars($empresa['ruc']) ?>)</title>
...
<body>
<div class="container">
    <h3 class="mb-4">🧾 Facturas de <strong>COMPRAS</strong> de <?= htmlspecialchars($empresa['razon_social']) ?> (<?= htmlspecialchars($empresa['ruc']) ?>)</h3>
